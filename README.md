...
| 功能     | 命令                                                                  | 说明                                             |
| ------ | ------------------------------------------------------------------- | ---------------------------------------------- |
| 订阅用户   | `bgm订阅用户 <用户名>` / `bangumi订阅用户 <用户名>`                               | 校验用户是否存在，并将其加入当前群的订阅列表，同时缓存用户昵称，返回用户名、昵称和主页链接。 |
| 取消订阅用户 | `bgm取消订阅用户 <用户名>` / `bangumi取消订阅用户 <用户名>`                           | 将指定用户从当前群的订阅列表中移除。                             |
| 查看订阅列表 | `bgm订阅列表` / `bangumi订阅列表`                                           | 查看当前群已订阅的 Bangumi 用户列表。                        |
| 更新昵称缓存 | `bgm更新昵称 <用户名>` / `bangumi更新昵称 <用户名>` / `bgm update nickname <用户名>` | 手动刷新指定用户的昵称缓存。                                 |
